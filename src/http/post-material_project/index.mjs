...
export const MaterialProject = new ORMClass({ collection: "MaterialProject" });
...
async function reply(req) {
  try {
    ...
    if (action === "create") {
      let result = await MaterialProject.create({
        data: {
          ...(payload || {}),
    ...
    if (action === "listAll") {
      let result = await MaterialProject.listAll({});
    ...
    if (action === "remove") {
      let result = await MaterialProject.remove({ oid: payload.oid });
    ...
    if (action === "get") {
      let result = await MaterialProject.get({ oid: payload.oid });
    ...
    if (action === "update") {
      let result = await MaterialProject.update({ data: payload });
